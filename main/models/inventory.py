...
class Inventory(models.Model):
    ...
    name = models.CharField(verbose_name="型式", max_length=256)
    # price = models.IntegerField(verbose_name="表示価格",default=0)
    # condition = models.CharField(verbose_name="程度", choices=CONDITION, max_length=2, default='5')
    ...
    account = models.ForeignKey(User, on_delete=models.CASCADE, blank=True, null=True, verbose_name="仕入担当")
    ...
    seller = models.ForeignKey('Client', on_delete=models.SET_NULL, blank=True, null=True, verbose_name="仕入先", related_name="seller_client")
    ...
    buyer = models.ForeignKey('Client', on_delete=models.SET_NULL, blank=True, null=True, verbose_name="販売先", related_name="buyer_client")
    ...
    price_month_day = models.IntegerField(verbose_name="月極日単価",default=0)
    ...
    def __str__(self):
        return '会社:'+self.company.name+' ID:'+str(self.id)+' ステータス:'+self.get_status_display()+' 型式:'+self.name+' 号機:'+str(self.serial_no)+' 分類:'+self.type.name+' メーカー:'+self.manufacturer.name
